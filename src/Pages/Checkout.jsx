import { Box, Center, Button, Img, Text } from "@chakra-ui/react";
import { QuestionOutlineIcon } from "@chakra-ui/icons";

import React, { useState } from "react";
import { CartContext } from "../context/cartcontext/CartContext";
import { useNavigate } from "react-router-dom";
import { Spacer, Flex, Heading, Table, Tr, Td } from "@chakra-ui/react";
      ...
      <Box w={"300px"} m={"auto"} position={"absolute"} left={"10px"}>
        ...
        <Box>
          ...
          >
            ...
            />
            <Text color={"red"} fontWeight={"bold"} fontSize={"14px"} mt="8px">
              Apply Offers & Deals
            </Text>
            <Button
              w={"60px"}
        ...
        <Box mt={"10px"} boxShadow={"md"} h={"100px"}>
          <Flex m={"10px"}>
            <input type="checkbox" checked={value} onChange={checkboxHandler} />
            <Box w={"180px"} ml={"10px"}>
              <Text fontSize={"14px"}>Donate ₹5.00 Tick to Add Hope.</Text>
              <Text fontSize={"12px"}>
        ...
        >
          <Flex
            ...
            color={"white"}
          >
            <Text fontWeight={"bold"}>Continue to Payment</Text>
            <Box>₹{!value ? checkout : (Number(checkout) + 5).toFixed(2)}</Box>
          </Flex>
        </Box>
      </Box>
    </>
